...
UOpenFiles::openFileDialog(const FString& dialogTitle, 
  ...
  //to allow dialog resizing and file browser styling, 
  //and to specify that the directory and file must exist.
	ofn.Flags = OFN_HIDEREADONLY | OFN_ENABLESIZING | OFN_EXPLORER;
  ofn.Flags |= OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST;

	if (flags & dialogFlags::e_multiple) {
  ...
  isSuccess = !!::GetOpenFileName(&ofn);
	...
	if (isSuccess) {
		if (flags & dialogFlags::e_multiple) {
      //When selecting multiple files, the returned string is a nullptr-delimited list
      //where the first element is the directory and the remaining elements are filenames.
    ...
    //Gets the index of the filter selected by the user in the variable
		...
		FString extension = cleanExtensionList.IsValidIndex
                        (outFilterIndex) ? cleanExtensionList[outFilterIndex] : TEXT("");
